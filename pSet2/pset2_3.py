# -*- coding: utf-8 -*-
"""
Created on Tue Dec  5 11:31:43 2017
...
so your observations from running this experiment on the grading system might 
...
decimalMonthsInAYear = 12.0

monthlyInterestRate = annualInterestRate / decimalMonthsInAYear
monthlyLowerBoundPay = balance / decimalMonthsInAYear
monthlyPaymentUpperBound = (balance * (1 + monthlyInterestRate)**12) / decimalMonthsInAYear

halfPay = (monthlyLowerBoundPay + monthlyPaymentUpperBound) / 2
epsilon = 0.01
newBalance = balance
...
while abs(0 - newBalance) >= epsilon:
    newBalance = balance
    for i in range(1, 13):
        newBalance = round((newBalance - halfPay) * (1 + monthlyInterestRate), 2)
    if newBalance >= 0:
        monthlyLowerBoundPay = halfPay
    else:
        monthlyPaymentUpperBound = halfPay
    halfPay = (monthlyLowerBoundPay + monthlyPaymentUpperBound) / 2
print("Lowest pay", round(halfPay, 2))
